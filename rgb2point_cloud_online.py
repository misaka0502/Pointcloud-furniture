...
def plane_segmentation(pcd: o3d.geometry.PointCloud) -> o3d.geometry.PointCloud:
    """
        利用RANSAC算法进行点云平面分割
    """
    ...
    plane_model, inliers = pcd.segment_plane(distance_threshold=0.001,
                                                    ...
                                                    num_iterations=1000)
    # inliers 是属于平面的点的索引
    # select_by_index 的 invert=True 参数表示选择所有“不是”内点的点
    object_pcd = pcd.select_by_index(inliers, invert=True)
    return object_pcd

def filter_background(pcd: o3d.geometry.PointCloud) -> o3d.geometry.PointCloud:
    """
...
if __name__ == '__main__':
    ...
    # 设置着色器。'defaultUnlit' 意味着颜色将是纯粹的顶点颜色，不受光照影响。
    mat.shader = "defaultUnlit"
    # 设置点的大小！这个值以屏幕像素为单位。
    mat.point_size = 3.0 # 将点的大小设置为5个像素
    ...
    #     {"name": "PointCloud", "geometry": point_cloud, "material": mat}
    # ], bg_color=(1.0, 1.0, 1.0, 1.0), show_skybox=False)