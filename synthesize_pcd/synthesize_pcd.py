...
def main():
    ...
    furniture = Furniture(ASEET_PATH, device='cuda:3', downsample_voxel_size=0.01)
    ...
    for i in range(400):
        ...
        part_pose['square_table_top'] = torch.tensor(data['parts_poses'][i, 0:7], device=furniture.device).unsqueeze(0).expand(1024, -1)
        part_pose['square_table_leg1'] = torch.tensor(data['parts_poses'][i, 7:14], device=furniture.device).unsqueeze(0).expand(1024, -1)
        part_pose['square_table_leg2'] = torch.tensor(data['parts_poses'][i, 14:21], device=furniture.device).unsqueeze(0).expand(1024, -1)
        part_pose['square_table_leg3'] = torch.tensor(data['parts_poses'][i, 21:28], device=furniture.device).unsqueeze(0).expand(1024, -1)
        part_pose['square_table_leg4'] = torch.tensor(data['parts_poses'][i, 28:35], device=furniture.device).unsqueeze(0).expand(1024, -1)
        if i == 0:
        ...
        # 降采样
        pcds_sampled = sample_points(torch.cat(list(furniture.parts_pcds_world.values())), sample_num=1024)
        if i == 0:
